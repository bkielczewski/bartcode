...
import uk.co.bartcode.service.article.Article
import java.util.*
...
interface PostRepository : JpaRepository<Post, Long> {
    ...
    fun findByYear(@Param("year") year: Int, pageable: Pageable): Page<Post>

    @RestResource(path = "yearMonth", rel = "yearMonth")
    @Query("SELECT p FROM Post p " +
            "WHERE FUNCTION('YEAR', p.published) = ?1 " +
            "AND FUNCTION('MONTH', p.published) = ?2")
    fun findByYearAndMonth(@Param("year") year: Int, @Param("month") month: Int, pageable: Pageable): Page<Post>

    @RestResource(path = "tag", rel = "tag")
    fun findByTags(@Param("tag") tag: String, pageable: Pageable): Page<Post>
